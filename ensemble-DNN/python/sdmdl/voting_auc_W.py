...
path_dir='F:/github/bird/result/rsep/도롱뇽/DMLP'
    ...
    for count in range(0, 1, 1):
        ...
        for list in forder_list:
            try:
                ...
                true_data=pd.read_csv("%s/%s/data1/Hynobius_leechii_total_env_dataframe_test.csv" % (path_dir, folder[j]), engine='python')['present/pseudo_absent']
        ...
        total_preds = pd.concat(
            [(weight[0]*round((csv_tests[0]))),(weight[1]*round((csv_tests[1]))),(weight[2]*round((csv_tests[2]))),
             (weight[3]*round((csv_tests[3]))),(weight[4]*round((csv_tests[4]))),(weight[5]*round((csv_tests[5]))),
             (weight[6]*round((csv_tests[6]))),(weight[7]*round((csv_tests[7]))),(weight[8]*round((csv_tests[8]))),
             (weight[9]*round((csv_tests[9]))),],
            axis=1)
