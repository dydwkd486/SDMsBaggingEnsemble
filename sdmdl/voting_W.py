...
path_dir='F:/github/bird/result/청개구리/DMLP'
folder= os.listdir(path_dir)
print(folder)
for count in range(4,-5,-1):
    Sorensen =[]
    OPR = []
    UPR = []
    Sensitivity = []
    ...
    for j in range(4,5,1):
        ...
        for list in forder_list:
            try:
                ...
                weight.append(
                    pd.read_csv("%s/%s/%s/results1/_DNN_performance/DNN_eval.txt" % (path_dir, folder[j], list),
                                engine='python', sep='\t').tss[0])
        ...
        total_preds = pd.concat(
            [(weight[0]*round(np.clip(csv_tests[0] - count / 10, 0, 1))),(weight[1]*round(np.clip(csv_tests[1] - count / 10, 0, 1))),(weight[2]*round(np.clip(csv_tests[2] - count / 10, 0, 1))),
             (weight[3]*round(np.clip(csv_tests[3] - count / 10, 0, 1))),(weight[4]*round(np.clip(csv_tests[4] - count / 10, 0, 1))),(weight[5]*round(np.clip(csv_tests[5] - count / 10, 0, 1))),
             (weight[6]*round(np.clip(csv_tests[6] - count / 10, 0, 1))),(weight[7]*round(np.clip(csv_tests[7] - count / 10, 0, 1))),(weight[8]*round(np.clip(csv_tests[8] - count / 10, 0, 1))),
             (weight[9]*round(np.clip(csv_tests[9] - count / 10, 0, 1))),],
            axis=1)

        total_preds = pd.DataFrame(np.transpose(total_preds))
        # print(total_preds)
        # print(total_preds)
        # total_preds=round(np.clip(csv_tests[0] - count / 10, 0, 1))

        total_preds = total_preds.sum(axis=0)
        ...
        # print("auc :", roc_auc_score(true_data, preds_result))
