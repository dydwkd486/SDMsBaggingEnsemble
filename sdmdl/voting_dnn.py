...
path_dir='F:/github/bird/result/rs/도롱뇽/MLP'
...
for count in range(4,-5,-1):
    ...
    for j in range(4,5,1):
        ...
        try:
            ...
            csv_tests.append(pd.read_csv("%s/%s/_DNN_performance/pred.txt" % (path_dir, folder[j]), engine='python',sep='\t',names=['%s' % folder[j]]))
            true_data=pd.read_csv("%s/%s/Hynobius_leechii_total_env_dataframe_test.csv" % (path_dir, folder[j]), engine='python')['present/pseudo_absent']
        ...
        # print(total_preds)
        total_preds =pd.DataFrame(csv_tests[0])
